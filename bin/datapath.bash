...
--source|-s gsiftpSourceUrl
			...
			This URL just consists of a scheme and an authority (see
			RFC3986 for details).
...
retrieveDataPaths()
{
	# retrieves data paths available on [remote] repository
	#
	# usage:
...
reindexDataPaths()
{
	# reindex (=(re)create index files for dpaths) data paths
	#
	# usage:
...
# RETRIEVE mode
	...
	if ! use globus-url-copy tar; then
